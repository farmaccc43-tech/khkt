// LOAD STATE
let progress = JSON.parse(localStorage.getItem("progressLesson3")) || {
    read: false,
    video: false,
    exercise: false
};
...
// SAVE
function saveState() {
    localStorage.setItem("progressLesson3", JSON.stringify(progress));
}
...
document.getElementById("mindmapBtn").addEventListener("click", function () {
    window.location.href = "mindmap2.html";
});
